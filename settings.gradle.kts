/*
 * This file was generated by the Gradle 'init' task.
 *
 * The settings file is used to specify which projects to include in your build.
 * For more detailed information on multi-project builds, please refer to https://docs.gradle.org/8.12.1/userguide/multi_project_builds.html in the Gradle documentation.
 */

plugins {
    // Apply the foojay-resolver plugin to allow automatic download of JDKs
    id("org.gradle.toolchains.foojay-resolver-convention") version "0.8.0"
}

rootProject.name = "FastTicketing"
include("app")

include("common")
include("user-service")
include("performance-service")
include("ticket-service")
include("payment-service")

project(":common").projectDir = file("modules/common")
project(":user-service").projectDir = file("modules/user-service")
project(":performance-service").projectDir = file("modules/performance-service")
project(":ticket-service").projectDir = file("modules/ticket-service")
project(":payment-service").projectDir = file("modules/payment-service")